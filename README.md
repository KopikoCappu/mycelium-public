...
<img src="https://raw.githubusercontent.com/KopikoCappu/Mycelium/main/assets/logo.png" width="72" alt="Mycelium" />
...
```
Without Mycelium:  agent reads 40 files  →  ~32,000 tokens used
With Mycelium:     agent reads 4 files   →  ~3,200 tokens used
```
...
- **Flow dots** — select a node and dots animate to show dependency direction. Lime flows toward you (imports). Pink flows away (imported by).
...
| Double-click canvas | Fit graph to screen |
| Shift+click | Multi-select |
| Right-click node | Hide, pin, isolate subtree |
| `/` | Focus search |
| `Esc` | Clear selection → subtree → search |

**Features**

- Task input in topbar runs `/preflight` and highlights relevant files
- History tab shows all sessions — click any file to jump to it in the graph
- Right-click → Pin node locks position across reloads
- Right-click → Show only subtree isolates a file's full dependency tree
- Click any description in the sidebar to edit it inline
- `{ } MD` button copies node info as a markdown block for Claude/Cursor
- Settings → Appearance has colorblind-friendly flow dot presets
...
- **`CLAUDE.md`** — tells Claude Code how to use Mycelium before starting any task
- **`.mcp.json`** — MCP server config so Claude Code finds the endpoints

Add this to your system prompt or `CLAUDE.md`:

```markdown
Before editing any file, call:
GET http://localhost:47821/preflight?task=<your task>

Read only the files returned. The contextSummary field is ready to use directly.
```
...
If [`codebase-memory-mcp`](https://github.com/codebase-memory-mcp/codebase-memory-mcp) is installed, Mycelium uses it automatically for structural indexing across 150+ languages — Python, Go, Rust, Java, C++, and more — plus full call graph tracing between functions.
